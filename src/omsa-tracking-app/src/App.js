...
// Starting components
    function useBusData() {
      ...
      useEffect(() => {
        ...
        .then(data => {
          const buses = data.payload.map(bus => ({
            busNumber: `B${bus.id}M`,
            passengers: bus.currentPassengers,
            capacity: bus.capacity,
            status: bus.timeToArrival === "00:00:00" || bus.timeToArrival.includes("-") ? 'Vencida' : null,
            eta: `${bus.timeToArrival} min`,
            location: {
      ...
      return (
        <div className="absolute top-0 left-0 w-full min-h-[482px]">
          <APIProvider apiKey={''}>
            ...
            {busData.map(bus => (
              <CustomMarker lat={bus.location.latitude} lng={bus.location.longitude} />
            ))}           
